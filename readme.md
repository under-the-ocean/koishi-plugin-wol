...
plugins:
  wolpc:
    ...
    allowLegacyGlobalDevices: false   # 是否读取旧版未绑定用户的全局设备
```
...
| `userIsolation` | boolean | true | 启用用户隔离。开启后，每个用户只能管理自己的设备 |
| `allowLegacyGlobalDevices` | boolean | false | 兼容旧数据。开启后，如果用户没有同名设备，会尝试读取未绑定用户的旧设备 |
